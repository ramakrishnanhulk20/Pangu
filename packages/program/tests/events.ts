...
describe("events", () => {
  it("reports the sale, the wallet, the amount and the running total on a buy", async () => {
    ...
    assert.isTrue(bought.mint.equals(env.mint), "the event does not name its sale");
  ...
  it("reports the sale, the wallet, the amount and the running total on a sell", async () => {
    const env = await setupSale({ cap: 1_000n });
    const buyer = Keypair.generate();
    fund(env, buyer.publicKey);
    mustSucceed(await openBuyerRecord(env, buyer));
    ...
    assert.isTrue(sold.mint.equals(env.mint), "the event does not name its sale");

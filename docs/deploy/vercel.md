...
| --- | --- | --- |
| `NEXT_PUBLIC_DEVNET_RPC_URL` | `packages/web/lib/solana.ts`, every server and client read of devnet | A keyed devnet RPC endpoint (Helius, QuickNode, Triton, or similar). Left unset, the app falls back to the public `https://api.devnet.solana.com`, which `docs/measurements/web-scaffold.md` measured rate-limiting a page load out to 8 seconds under concurrent reads. The attack ledger sizes each row against this endpoint, which takes 20 to 70 seconds a row on the public node and a few seconds on a keyed one, so set it before judging. The team fills in the actual endpoint and key. |

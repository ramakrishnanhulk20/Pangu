...
The secrets in this list are the key inside `DEVNET_RPC_URL`, `PYTH_API_KEY` and `DEMO_DOLLAR_MINT_AUTHORITY`. All three are read on the server and nowhere else. `DEMO_DOLLAR_MINT_AUTHORITY` is a devnet demo key holding nothing of value. `NEXT_PUBLIC_DEVNET_RPC_URL` is not a secret by design: whatever it holds is public the moment the site is.

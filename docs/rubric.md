...
| C5 | A holder can always sell back to the pool. No missing account or stale rule can block a sell. | `execute.rs`, the sell branch reads only the source owner's record if it exists | `tests/execute.ts` sell cases, WO-5's 1,330 simulated exit sells | [`WiANjhTm`](https://explorer.solana.com/tx/WiANjhTmTM6H9K1N23axK8GX5oEnYnFhqkgzNBHSdkUo4Gr2arvhqBsz8vequdXxPjNGpPgd1eQ8XPTwToj6BXo?cluster=devnet), a revoked wallet's sell goes through |
...
| C12 | Scripts never hold or print a private key; every address that receives money is shown to the signer before signing and read back after. | `packages/scripts/src/wallets.ts`, `chain.ts` | `packages/scripts` test suite (19 tests) | devnet-run.md, no key ever printed in any of the five commands' output |
...
From `PLAN.md`, "What Meteora is pushing this month".
...
```
Prints `rust: test result: ok. 26 passed` and a mocha summary of 118 passing, then
exits 0.
...
```
and exits 0. `npm run launch`, `npm run seed`, and `npm run graduate` set up and
...
- The upgrade key holder (Ram, stated openly) can change the program while a
  sale is live, unless the key is later given up. That trade-off and its reasons
  are in `PLAN.md` and `DECISIONS.md`.
...
- The price band's oracle source is mid-migration. It shipped and was fully
  proven on Switchboard On-Demand (forgery attempts refused on devnet, see
  `docs/security/attacks/forged-quote.md`), then rebuilt on Pyth after
  Switchboard announced it shuts down on 25 September 2026. The Pyth version
  (WO-13, WO-14, WO-15) is mid-build as of this writing; the devnet band runs
  proven so far in this document are on the retired Switchboard build.
- The Pyth price key Ram holds is trial access, extended by Pyth support to 5
  October 2026, which covers judging but not a production issuer. A real issuer
  would need its own Pyth key.
- `PriceOutsideBand` (a buy refused for going above the ceiling) has not been
  landed as a live devnet or mainnet transaction. It needs a dollar-priced sale
  with real trading volume to reach the ceiling; the devnet demo used a
  SOL-priced sale, where the ceiling sits eight orders of magnitude away from
  any affordable buy. The rule is proven in the fork test instead.
  see `docs/measurements/devnet-run.md`, "The ceiling itself".
- The demo credential used for the attestation approval mode is self-issued.
  ...
  here.
- Mainnet deploy has not happened. Every "mainnet: pending" cell in section 1 is
  honest about that; the program is proven end to end on devnet only as of this
  writing.

## 6. Pages read for this rubric

`reference/stocklana-program.md`, `PLAN.md`, `ARCHITECTURE.md`,
...
`docs/deployments.md`, `STATE.md`, `packages/scripts/package.json`,

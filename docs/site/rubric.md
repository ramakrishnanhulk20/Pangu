...
| Judging: "technical soundness" | 31 Rust tests, 141 unit tests (on solana-bankrun, against a v0 build of the same source, because bankrun cannot load SBPF v3), 38 forked mainnet steps (the program's full sale life against real cloned DBC, DAMM v2 and attestation programs, on the SBPF v3 build devnet runs; the SDK's own fork suite adds its 16 steps on the same validator and build, run the same day through a keyed mainnet node), 178 SDK tests, 126 script tests, 10 randomized attack sequences of 6,000 operations, eight live devnet runs with every refusal landed as a real transaction and all three access modes attacked on the live program, a full code review with findings fixed, the program audit of 23 September that added C18 to C21, and the code review of 24 September that added C22 to C25. | The four page proofs, launch, buy and sell, verify and portfolio, each a script that drives the real page on devnet with a throwaway wallet and checks every line against the chain; and the forked-mainnet rehearsal, which ran the verified mainnet build's deploy, sales in USDC and AAPLx, the Squads handover, and the mainnet build of the app. | see section 4 for exact commands | all of the above | test-counts.md, devnet-run.md, fork-test.md, random-sequences.md, mainnet-rehearsal.md, and `launch-devnet.txt`, `metadata-devnet.txt`, `sale-devnet.txt`, `verify-devnet.txt`, `portfolio-devnet.txt`, `network-fork.txt` |
...
fifth deploy of 23 September 2026, and their evidence is from the sixth run.
C18's dollar list came with the sixth deploy of 23 September 2026, and its
refusal landed in the eighth run. C22 to C25, and
...
```
Prints `rust: test result: ok. 31 passed` and a mocha summary of 141 passing, then
exits 0.
...
```
Prints 38 passing fork tests, including the AAPLx-paid launch and the DAMM v2
migration, and exits 0. Takes about 5 minutes; run detached and tail the log for

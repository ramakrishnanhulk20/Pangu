...
Twenty-one rules, C1 to C21, each with its proof beside it. The fourteen first rules on the program (C1 to C14) are each proven by a named test, and most also by a real, refused transaction on Solana devnet. The three rules on the app (C15 to C17) are proven by recorded devnet runs or, for C17, by inspection of the code. The four program rules added in the rules v2 build (C18 to C21) are proven by named tests and by the sixth run, which opened both of its sales under them; C18's own refusal, `BandNeedsDollarQuote`, then landed on devnet in the eighth run. The other three refusals have not been sent on devnet, because every demo sale was set up to pass them. The full table, with the file, the function, the test and the transaction for each rule, is on the rubric page.
...
| C15 | The devnet demo key is the only key server code holds, used by two routes. The demo dollars button mints only the demo dollar, at most once a wallet an hour and ten times a minute. The price refresh posts only Pyth price updates for Pangu sales with a price band that the chain lists, at most one a minute and only when the stored price is over ten minutes old. The key never leaves the server. | a race of 12 wallets on devnet: 10 let through the reservation, the other 2 refused; two price refresh calls in a row on devnet, one post then none |
...
| C18 | A sale with a price ceiling is paid in a dollar the program lists for its network (USDC on mainnet; devnet USDC and the demo dollar on devnet), and every sale's paying token is checked and stored. | tests refusing a band on a sale priced in SOL; devnet transaction, `BandNeedsDollarQuote`, a ceiling on a token that looks like a dollar and is not on the list; the fork rehearsal refused a ceiling on the demo dollar under the mainnet build |
...
| C21 | The offering period is fixed when the sale opens. While it runs every rule holds; when it ends every rule lifts. | five offering period tests and a forked-mainnet step where the rules lifted at the end; PBAND2 and PAAPLX opened with an end of 7 October 2026 and every rule held on them afterwards |
...
- **The upgrade key.** Pangu's upgrade key is held by the team's wallet and stated openly. It could be used to change the program's code during a live sale; the team keeps it so a bug found during judging can be fixed. On mainnet the plan is to move it to a Squads v4 multisig, two of three, and freeze the program for good once an independent audit is done. The plan and its rehearsal are on the mainnet page.
- **The dollar list is a list of addresses.** A ceiling is allowed only on the exact dollar tokens the program lists. It does not check that a listed stablecoin still holds its peg.
- **The freeze check covers the issuer's own key.** A freeze authority held by another wallet the issuer controls cannot be told apart from a stablecoin issuer's.
- **This has not been independently audited by a third party.** It has been reviewed by an automated code-review pass and by the team building it, tested with 31 Rust tests, 137 litesvm tests, 37 steps against real cloned Meteora and attestation programs, 146 SDK tests, 109 script tests, and 6,000 randomized attack operations, rehearsed end to end on a forked copy of mainnet, and proven with real refused transactions on a live network. That is a strong bar for a hackathon submission, but it is not the same thing as a paid third-party security audit.

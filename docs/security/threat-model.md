...
| C15 | The one server code path that holds a key is the demo key, a devnet key that holds only devnet SOL, read by one loader and used by two routes. The demo dollars route mints only the demo dollar token, at most one grant per wallet per hour and ten a minute, with both slots reserved before any await so concurrent requests cannot pass the limit together and released when a grant fails. The price refresh route posts only Pyth price updates, only for Pangu sales with a price band that the chain lists (the directory of C17), only when the stored price is over ten minutes old, one post at a time and at most one a minute across the process, and nothing when Pyth has no price newer than the sale's age limit; each post costs about 0.000035 SOL. Neither the demo key nor the Hermes key ever reaches a public variable, a client import, a response or a log. Upheld by `demoKey`, `reserve`, `keep` and `release` in `packages/web/lib/demo-dollars.ts`, imported only by `app/api/break/dollars/route.ts` and `packages/web/lib/price-refresh.ts`, and by `refreshIfStale` and `hermesPublishTime` in `packages/web/lib/price-refresh.ts` with the `openedSales` band check in `app/api/price/refresh/route.ts`. Found in the finishing sweep review, 23 Sep 2026, widened for the price refresh the same day. |
...
| C17 | No public route lets an outsider spend the RPC budget the live numbers depend on. The price route answers only for mints in the app's own sale list and serves each answer from a ten second cache, and the pulse and readout readings are shared across requests. The readout and price refresh routes answer for any Pangu sale on the chain and nothing else: the set of mints is the chain's own list of sales, bounded by what the program holds, read in one shared directory read of about six calls at most every thirty seconds; a mint that list does not know reads it again at most every five seconds, so made-up mints cost devnet one read per window, not one per ask. The refresh route takes only a sale with a price band and keeps C15's limits: one post at a time, at most one a minute across the process. Upheld by the `openedSales` check and the cache in `app/api/price/[mint]/route.ts`, the shared reading in `packages/web/lib/pulse.ts`, `readDirectory` and `findSale` in `packages/web/lib/directory.ts`, `readReadout` in `packages/web/lib/readout.ts`, and the band check in `app/api/price/refresh/route.ts`. Found in the finishing sweep review, 23 Sep 2026, widened to every sale on the chain for the sale pages the same day. |
| C18 | A price ceiling is allowed only when buyers pay in a dollar token on the program's list for its network (USDC on mainnet; devnet USDC and the demo dollar on devnet). Upheld by `check_band` and `DOLLAR_MINTS`. |
| C19 | The issuer cannot freeze the paying token. Whoever can freeze it can freeze the pool's paying token vault or a seller's account, and a frozen account cannot be paid, which would make the exit in C5 the issuer's to close. create_sale refuses a paying token whose freeze authority is the issuer with `IssuerControlsPayingToken`. Covers the issuer's own key; an authority held by another wallet the issuer controls cannot be told apart from a stablecoin issuer's and is a named non-goal. Upheld by `handle_create_sale` in `packages/program/programs/pangu/src/instructions/create_sale.rs`. Found in the program audit of 23 Sep 2026. |
| C20 | The cap is below what the curve sells. A cap at or above the launch template's `swap_base_amount` would let one wallet buy the whole sale, which is the one outcome a per-wallet cap exists to stop; create_sale refuses it with `CapCoversWholeSale`. The curve's own supply is the bound rather than the mint's total, because the tokens held back for graduation never leave the vault through a buy. Upheld by `handle_create_sale` and `read_swap_base_amount` in `packages/program/programs/pangu/src/instructions/create_sale.rs`, with the offset checked against a mainnet template. Found in the program audit of 23 Sep 2026. |
| C21 | The offering period is fixed at creation and cannot be changed; while it runs every rule holds, and when it ends every rule lifts so a curve that never fills cannot lock the token for good. Upheld by create_sale (EndInThePast) and the early return in execute. Only rules in a layout this build reads can end: an older layout never counts as ended. Upheld by `handle_create_sale` in `create_sale.rs`, `handle_execute` in `execute.rs`, `SaleRules::offering_is_over` in `state.rs` and `handle_close_buyer_record` in `close_buyer_record.rs`, all under `packages/program/programs/pangu/src`. Found in the program audit of 23 Sep 2026, decided by the founder the same day. |

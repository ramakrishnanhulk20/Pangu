...
| C18 | A banded sale is priced in a dollar token. The band compares the curve with a stock price in dollars, so a sale whose buyers pay in wrapped SOL would measure two numbers in different units and its ceiling would mean nothing; create_sale refuses it with `BandNeedsDollarQuote`. The paying token is the one the launch template names, proven for every sale, and stored in the rules. Covers wrapped SOL under both token programs, taken from the token crates; any other token that is not a dollar is not recognised, which is a named non-goal. Upheld by `check_band` and `handle_create_sale` in `packages/program/programs/pangu/src/instructions/create_sale.rs`. Found in the program audit of 23 Sep 2026. |
